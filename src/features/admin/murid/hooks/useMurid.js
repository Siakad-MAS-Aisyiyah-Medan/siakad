...
import { fetchKelasList } from '../../kelas/services/kelas.service';
...
export function useMurid() {
  ...
  const promoteMurid = async (murid) => {
    ...
    if (ppdbStatus === 'enrolled') {
      Swal.fire('Info', 'Calon siswa sudah pernah dipromosikan.', 'info');
      return;
    }

    if (ppdbStatus !== 'accepted') {
      Swal.fire(
        'Belum Dapat Dipromosikan',
        'PPDB harus berstatus Diterima (accepted) terlebih dahulu. Gunakan menu Verifikasi PPDB.',
        'warning'
      );
      return;
    }
