...
import { fetchAdminPendaftarDetail } from '../services/ppdbApi';
import { BERKAS_LABELS } from '../services/ppdbApi';
...
export default function AdminDetailPendaftar() {
  ...
  return (
    <AdminPageShell>
      <motionDetail loading={loading} data={data} status={status} id={id} />
    </AdminPageShell>
  );
}

function motionDetail({ loading, data, status, id }) {
  return (
    <div className="data-panel">
      ...
      {loading ? (
        ...
        <motionContent data={data} status={status} />
      )}
    </div>
  );
}

function motionContent({ data, status }) {
  return (
